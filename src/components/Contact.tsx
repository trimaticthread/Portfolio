...
const ContactForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (now - lastSubmitTime < RATE_LIMIT_MS) {
      ...
      toast.error(`Çok hızlı mesaj gönderiyorsunuz. ${remainingTime} dakika sonra tekrar deneyin.`);
      return;
    }
    ...
    if (hasSpam) {
      toast.error('🚫 Mesajınız spam olarak algılandı, lütfen farklı bir içerik yazın');
      return;
    }
    ...
    try {
      ...
      toast.success('🎉 Mesajınız başarıyla gönderildi! En kısa sürede yanıtlayacağım.');
      ...
      toast.error(`🚨 ${errorMessage} Lütfen e-posta ile direkt iletişime geçin: toprakgulec34@gmail.com`);
                  ...
                  <form id="contact-form" onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                      <div>
                        ...
                        <Input
                          ...
                          placeholder="Adınız ve soyadınız (max 50 karakter)"
                        />
                      </div>
                      <div>
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={handleInputChange}
                        className="bg-gray-800/60 border-2 border-gray-600 text-white placeholder:text-gray-400 focus:border-emerald-500 focus:ring-emerald-500/20 transition-colors h-10 sm:h-12 text-sm sm:text-base rounded-lg"
                        placeholder="Mesajınızın konusu (max 100 karakter)"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <Textarea
                        ...
                        placeholder="Mesajınızı buraya yazın... (min 10, max 1000 karakter)"
                      />
                      <div className="flex justify-between items-center mt-2">
                        <span className={`text-xs ${formData.message.length < 10 ? 'text-red-400' : 'text-gray-500'}`}>
                          {formData.message.length < 10 ? `En az ${10 - formData.message.length} karakter daha` : '✓ Yeterli'}
                        </span>
